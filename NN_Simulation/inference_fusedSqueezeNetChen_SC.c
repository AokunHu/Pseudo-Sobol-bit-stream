...
)
{
	...
	for (i = start_idx; i <= end_idx; i++)
	{
		...
		prediction=SqueezeNetChen_SC(input_data[i],weights_conv,bias_conv,weight_conv_fire,bias_conv_fire,Sobol_seq,len);
...
int main(void)
{
	...
	for(i=0;i<core_num;i++)
	{
		...
		// load_Pseudo_Random(Sobol_seq[i]);
	}
	...
	for(core = 0; core < core_num; core++)
		err_core[core] = run_core(core*num_pic/core_num,(core+1)*num_pic/core_num-1,input_data,label,weights_conv,bias_conv,weight_conv_fire,bias_conv_fire,Sobol_seq[core],len);
